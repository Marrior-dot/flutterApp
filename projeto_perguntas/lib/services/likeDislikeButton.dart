...
Future<Postagem> updateLikeDislike(String likeOrDislike, int button, int id,
    String content, List<dynamic> respostas) async {
...
Future<int> fetchLike(int id) async {
  final response = await http
      .get(Uri.parse('http://localhost:8000/api/postagemdetail/$id/'));
  if (response.statusCode == 200) {
    var postagemMap =
        Postagem.fromJson(jsonDecode(response.body) as Map<String, dynamic>);
    print(postagemMap.arquivo);
